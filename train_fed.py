...
class ClientThread(Thread):
    ...
    def run(self) -> None:
        train_dataset, val_dataset, test_dataset = get_dataset({self.client_idx: self.user_cluster_datas[self.client_idx]})
        ...
        test_dataloader = DataLoader(dataset=test_dataset, batch_size=16, shuffle=False,
                                     collate_fn=lambda x: ([i[0] for i in x], [i[1] for i in x]))
        client_train(self.sub_model, train_dataloader, val_dataloader, test_dataloader)

    def get_result(self):
        return self.sub_model


def client_prepare(train_dataset, val_dataset, test_dataset, model: nn.Module, embedding_lookups: dict):
    embedding_weights = {}
    embedding2idxes = {}
    ...
    sub_model = SparseMMOE(embedding_list=embedding_lookups.keys(), pretrained_embeddings=embedding_weights, embedding2idxes=embedding2idxes)
    for layer in model.state_dict().items():
        layer_name, layer_parameter = layer
        if layer_name.startswith('embeddings'):
            continue
        sub_model.load_state_dict(state_dict={layer_name: layer_parameter}, strict=False)

    return sub_model, embedding_weights, embedding2idxes


def merge_model(model: SparseMMOE, embedding_lookups: dict[str, EmbeddingLookup], submodels: [SparseMMOE]):
    # merge model
    submodel_num = len(submodels)
    for layer_name in model.state_dict().keys():
        if layer_name.startswith('embeddings'):
            continue
        if layer_name.endswith('num_batches_tracked'):
            continue
        model.state_dict()[layer_name] = 0
        for submodel in submodels:
            model.state_dict()[layer_name] += submodel.state_dict()[layer_name]
        model.state_dict()[layer_name] /= submodel_num
...
def client_train(model: nn.Module, train_dataloader: DataLoader, val_dataloader: DataLoader,
    ...
    for epoch in range(n_epochs):
        ...
        for features, labels in train_dataloader:
            ...
            loss_2 = loss_fn_2(label_2.float(), label_hat_2) * 0.015
            loss = loss_1 + loss_2
            loss.backward()
        ...
        # print(f'[client] task 1 train loss: {np.mean(epoch_loss[0]):.5f}, t'
    ...
    # print(f'[client] val task 1 rmse: {rmse_1:.3f}, val task 2 auc: {auc_2:.3f}')
...
def client_main(client_idx, user_cluster_datas, model, embedding_lookups):
    ...
    val_dataloader = DataLoader(dataset=val_dataset, batch_size=16, shuffle=False, drop_last=True,
                                collate_fn=lambda x: ([i[0] for i in x], [i[1] for i in x]))
    test_dataloader = DataLoader(dataset=test_dataset, batch_size=16, shuffle=False, drop_last=True,
                                 collate_fn=lambda x: ([i[0] for i in x], [i[1] for i in x]))
...
def server_train():
    ...
    for embedding_key in embedding_list:
        embedding_weights[embedding_key], embedding2idxes[embedding_key] = \
            embedding_lookups[embedding_key].get_embeddings([train_dataset, val_dataset, test_dataset])
    model = SparseMMOE(embedding_list=embedding_lookups.keys(), pretrained_embeddings=embedding_weights, embedding2idxes=embedding2idxes)
    ...
    for i in range(epoch):
        ...
        for client_idx in client_idx_list[:client_num]:
            client_thread = ClientThread(client_idx, user_cluster_datas, model, embedding_lookups)
            client_thread.start()
        ...
        merge_model(model, embedding_lookups, sub_models)
        train_rmse_1, train_auc_2 = server_test(model, embedding_lookups, train_dataset)
        rmse_1, auc_2 = server_test(model, embedding_lookups, val_dataset)
    ...
    rmse_1, auc_2 = server_test(model, embedding_lookups, test_dataset)
...
def server_test(model: SparseMMOE, embedding_lookups: dict[str, EmbeddingLookup], dataset: MovielensDataset):
    for embedding_key in embedding_lookups.keys():
        ...
        model.embeddings[embedding_key] = nn.Embedding.from_pretrained(embeddings=torch.FloatTensor(embedding_weights), freeze=False)
