...
if __name__ == '__main__':
    ...
    embedding_maps = {}
    for embedding_key in embedding_list:
        embedding_weights[embedding_key], embedding_maps[embedding_key] = \
            embedding_lookups[embedding_key].get_embeddings([train_dataset, val_dataset, test_dataset])

    train_dataloader = DataLoader(dataset=train_dataset, batch_size=64, shuffle=True, collate_fn=lambda x: ([i[0] for i in x], [i[1] for i in x]))
    ...
    model = SparseMMOE(embedding_list=embedding_list, pretrained_embeddings=embedding_weights, embedding_maps=embedding_maps)

    train(model, train_dataloader, val_dataloader, test_dataloader)

    for embedding_key in embedding_list:
        embedding_lookups[embedding_key].update_embeddings(embedding_maps[embedding_key],
                                                           model.embeddings[embedding_key].weight.data)
