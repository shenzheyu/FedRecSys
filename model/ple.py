import torch
from model.layers import EmbeddingLayer, MultiLayerPerceptron
...
class PLEModel(torch.nn.Module):
    ...
    def __init__(self, categorical_field_dims, numerical_num, embed_dim, bottom_mlp_dims, tower_mlp_dims, task_num,
        ...
        for i in range(self.layers_num):
            if i == 0:
                self.expert.append(torch.nn.ModuleList(
                    [MultiLayerPerceptron(self.embed_output_dim, [bottom_mlp_dims[i]], dropout, output_layer=False) for
                     j in range(self.specific_expert_num * self.task_num + self.shared_expert_num)]))
            else:
                self.expert.append(torch.nn.ModuleList(
                    [MultiLayerPerceptron(bottom_mlp_dims[i - 1], [bottom_mlp_dims[i]], dropout, output_layer=False) for
                     j in range(self.specific_expert_num * self.task_num + self.shared_expert_num)]))
        ...
        for i in range(self.layers_num):
            ...
            gate_list = [torch.nn.Sequential(torch.nn.Linear(input_dim, shared_expert_num + specific_expert_num),
                                             torch.nn.Softmax(dim=1)) for j in range(self.task_num)]
            gate_list.append(
                torch.nn.Sequential(torch.nn.Linear(input_dim, shared_expert_num + task_num * specific_expert_num),
        ...
        for i in range(task_num):
            ...
            index_list.extend(range(task_num * self.specific_expert_num,
                                    task_num * self.specific_expert_num + self.shared_expert_num))
